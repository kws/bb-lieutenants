...
export function createTerrainMeshes(scene: Scene, terrain: TerrainWorld): Mesh[] {
  ...
  for (const surfaceId of terrain.getSurfaceIds()) {
    ...
    mesh.metadata = { ...(mesh.metadata ?? {}), terrainSurfaceId: surface.id };
  ...
  for (const surfaceId of terrain.getSurfaceIds()) {
    const surface = terrain.getSurface(surfaceId);
    if (surface?.kind !== "tunnel") continue;
    meshes.push(...createTunnelVisibilityMeshes(scene, terrain, surface));
  }
...
function createTunnelVisibilityMeshes(scene: Scene, terrain: TerrainWorld, surface: RuntimeSurface): Mesh[] {
  const meshes: Mesh[] = [];
  const cutaway = createTunnelCutawayMesh(scene, terrain, surface);
  meshes.push(cutaway);

  const portals = terrain.portals.filter((portal) => portal.kind === "tunnel-mouth" && portal.to.surfaceId === surface.id);
  for (const portal of portals) {
    meshes.push(...createTunnelPortalMeshes(scene, terrain, portal.from.x, portal.from.z, portal.to.x, portal.to.z, portal.id));
  }

  return meshes;
}

function createTunnelCutawayMesh(scene: Scene, terrain: TerrainWorld, surface: RuntimeSurface): Mesh {
  const positions: number[] = [];
  const indices: number[] = [];
  const normals: number[] = [];
  const uvs: number[] = [];
  ...
  const mesh = new Mesh(`terrain.tunnel.cutaway.${surface.id}`, scene);
  vertexData.applyToMesh(mesh);
  mesh.material = getTunnelCutawayMaterial(scene);
  mesh.isPickable = false;
  mesh.alwaysSelectAsActiveMesh = true;
  return mesh;
}

function createTunnelPortalMeshes(
  scene: Scene,
  ...
  const frameMaterial = getTunnelPortalMaterial(scene);
  const openingMaterial = getTunnelOpeningMaterial(scene);
  const meshes: Mesh[] = [];
  ...
  if (axisX) {
    makeBox(
      `terrain.tunnel.portal.${id}.left`,
      { width: 0.7, height: 2.6, depth: 0.5 },
      { x: fromX, y: base + 1.3, z: fromZ - 2.45 },
      frameMaterial,
    );
    makeBox(
      `terrain.tunnel.portal.${id}.right`,
      { width: 0.7, height: 2.6, depth: 0.5 },
      { x: fromX, y: base + 1.3, z: fromZ + 2.45 },
      frameMaterial,
    );
    makeBox(
      `terrain.tunnel.portal.${id}.top`,
      { width: 0.8, height: 0.55, depth: 5.4 },
      { x: fromX, y: base + 2.85, z: fromZ },
      frameMaterial,
    );
    makeBox(
      `terrain.tunnel.portal.${id}.opening`,
...
function getTunnelCutawayMaterial(scene: Scene): StandardMaterial {
  const name = "terrain.material.tunnel.cutaway";
  const existing = scene.getMaterialByName(name);
  if (existing instanceof StandardMaterial) return existing;
...
function getTunnelPortalMaterial(scene: Scene): StandardMaterial {
  const name = "terrain.material.tunnel.portal";
  const existing = scene.getMaterialByName(name);
  if (existing instanceof StandardMaterial) return existing;
...
function getTunnelOpeningMaterial(scene: Scene): StandardMaterial {
  const name = "terrain.material.tunnel.opening";
  const existing = scene.getMaterialByName(name);
  if (existing instanceof StandardMaterial) return existing;
